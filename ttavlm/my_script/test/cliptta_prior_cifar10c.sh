#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# CLIPTTA + dynamic class-prior (cliptta_prior) on CIFAR-10-C
# (gaussian_noise, severity 5). Ready to run on a vast.ai GPU instance.
#
# Usage:
#   bash ttavlm/my_script/test/run_cliptta_prior_cifar10c.sh
#
...
python -m ttavlm.main \
    --env vast \
    --exp_name cliptta_prior_cifar10c \
    --root "${ROOT}" \
    --dataroot "${DATAROOT}" \
    --save_root "${SAVE_ROOT}" \
